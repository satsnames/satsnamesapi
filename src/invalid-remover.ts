...
export class InvalidRemover {
  ...
  constructor(db: PrismaClient, lastIndex: number) {
    ...
    this.real = process.env.DRY_RUN === "false";
    this.removed = [];
  }
  ...
  static async create() {
    ...
    return new InvalidRemover(db, last);
  }
  ...
  async processInscription(reg: Inscription) {
    const validationResult = validateSnsInscription(reg.inscriptionContent);
    if (validationResult === false) {
      const id = reg.inscriptionId;
      logger.info(
        {
          id,
        },
      ...
      if (this.real) {
        await this.db.registration.delete({
          where: {
            inscriptionId: id,
          },
        });
